...
public:
	string nearMiss(vector <int> p1, vector <int> v1, vector <int> p2, vector <int> v2, int R) {
		double x1 = p1[0], y1 = p1[1], z1 = p1[2], x2 = p2[0], y2 = p2[1], z2 = p2[2];
		double vx1 = v1[0], vy1 = v1[1], vz1 = v1[2], vx2 = v2[0], vy2 = v2[1], vz2 = v2[2];
		double dist_prev, dist_cur;
		
		dist_prev = sqrt( (z1 - z2)*(z1 - z2) + (y1 - y2)*(y1 - y2) + (x1 - x2)*(x1 - x2) );
		dist_cur = dist_prev + 2*R;
		
		if( dist_cur - dist_prev <= R ) return "YES";
		
		double fac = 0.25;
		for( double i = 1; i < 1000; i += fac )
		{
			x1 += vx1*fac;
			y1 += vy1*fac;
			z1 += vz1*fac;
			...
			y2 += vy2*fac;
			z2 += vz2*fac;
			dist_cur = sqrt( (z1 - z2)*(z1 - z2) + (y1 - y2)*(y1 - y2) + (x1 - x2)*(x1 - x2) );
			if( dist_cur > dist_prev ) return "NO";
			if( dist_cur <= R ) return "YES";
			dist_prev = dist_cur;
		}
