...
public:
  bool can_delete( int i, int j, int k, vector <int> y )
  {
    if( i < k && k < j )
      if( min( y[i], y[j] ) < y[k] && max( y[i], y[j] ) > y[k] )
        return true;
    ...
    for( int i = 0; i < y.size(); i++ )
    {
      ...
      {
        c = 0;
        cout << i << "," << y[i] << " and " << j << "," << y[j] << endl;
        for( int k = 0; k < y.size(); k++ )
        {
          if( can_delete( i, j, k, y ) )
          {
            cout << " with " << k << "," << y[k] << endl << endl;
            c++;
          }
        }
        num = max( num, c );
