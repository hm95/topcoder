...
public:
	...
	int max_length( int index, int number, int* dp, int* sign, vector <int> sequence )
	{
		int maxv = 0;	
		
		for( int i = index - 1; i > 0; i-- )
		{
			if( number - sequence[i] == -1 * sign[i] ) maxv = max( maxv, dp[i] );
		}
	...
	int longestZigZag( vector <int> sequence )
	{
		...
		for( int i = 1; i < sequence.size(); i++ )
		{
			if( sequence[i] - sequence[i - 1] == 0 )
				  sequence.erase( sequence.begin() + i - 1, sequence.end() + i );
		...
		{
			dp[i] = max( 2, 1 + max_length( i, sequence[i], dp, sign, sequence ) ); 
